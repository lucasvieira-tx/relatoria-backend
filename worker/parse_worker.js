...
async function pickOneAndMarkProcessing() {
    ...
    const { data, error } = await supabaseAdmin.rpc("sql", { q: sql }).catch(() => ({ data: null, error: null }));
...
async function pickSimple() {
    const { data } = await supabaseAdmin.from("datasets").select("*").eq("status", "uploaded").order("created_at", { ascending: true }).limit(1);
    if (!data || data.length === 0) return null;

    const ds = data[0]
    const { error } = await supabaseAdmin.from("datasets").update({ status: "processing" }).eq("id", ds.id).eq("status", "uploaded");
    if (error) {
        console.warn("⚠️ [Worker] Could not mark dataset as processing:", error.message);
        return null;
    }
...
async function processDataset(ds) {
    ...
    // Download
    const { data: download, error: dlErr } = await supabaseAdmin.storage.from("datasets").download(ds.storage_path);
    if (dlErr) {
        console.error("❌ - [Worker] Download error", dlErr.message);
        await supabaseAdmin.from("datasets").update({ status: "invalid", error_message: dlErr.message }).eq("id", ds.id);
        return;
    }
    ...
    try {
        if (ext === ".csv" || ext === "") {
            result = await extractFromCSV(buffer, { sampleRows: 30, maxInspect: 2000 });
        ...
        await supabaseAdmin.from("datasets").update({ status: "invalid", error_message: err.message }).eq("id", ds.id);
        return;
    }
    ...
    if (sampleJson.length > MAX_SAMPLE_SIZE) {
        ...
        const { error: sampleErr } = await supabaseAdmin.storage.from("datasets_sample").upload(sample_path, Buffer.from(sampleJson), { contentType: "application/json" });
        if (sampleErr) {
            console.error("❌ - [Worker] Sample upload error", sampleErr.message);
            await supabaseAdmin.from("datasets").update({ status: "invalid", error_message: sampleErr.message }).eq("id", ds.id);
            return;
        }
    }
    ...
    const { error: updErr } = await supabaseAdmin.from("datasets").update(upd).eq("id", ds.id);
    if (updErr) {
        console.error("❌ - [Worker] Update error", updErr.message);
        await supabaseAdmin.from("datasets").update({ status: "invalid", error_message: updErr.message }).eq("id", ds.id);
        return;
    }
